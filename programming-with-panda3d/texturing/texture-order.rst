...
To specify the texture order, use
``TextureStage.setSort()`` on one or more of your
TextureStages. If you do not specify a sort value, the default sort value is
0. When the geometry is rendered, all of the textures are rendered in
increasing order of sort value, such that the largest sort value is rendered
on top. Thus, if you want to use Decal mode, for instance, to apply a texture
on top of a lower texture, it would be a good idea to use setSort() to give a
...
``TextureStage.setPriority()`` for this.
...
sort them in order by the setSort() value). Between two textures with the same
priority, Panda will prefer the one with the lower sort value. The default
priority is 0.
