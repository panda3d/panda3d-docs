...
.. only:: python
    ...
    specifically designed for scrolling through a large window. In fact, a pair of
    DirectScrollBars is used to implement the :ref:`directscrolledframe`, which
    manages this scrolling functionality automatically. (Because
    ...
    thumb by prefixing each of these parameters with the prefix "thumb_", e.g.
    ``thumb_frameSize``; similarly, you can
    control the look of the two scroll buttons by prefixing these with
    "incButton_" and "decButton_". You can retrieve or set the current position of
    the thumb with ``myScrollBar['value']``.
