...
.. only:: cpp
    ...
    download it from the `Apple developer
    website <https://developer.apple.com/>`__. You will need to register for an
    account.
    ...
    include files and to the Python include files. Note: only i386 arch is
    supported for Panda3D versions before 1.9.0, so for those versions you need to
    include the -arch i386 flag.
    ...
    Please change the paths in these commands to the appropriate locations. A list
