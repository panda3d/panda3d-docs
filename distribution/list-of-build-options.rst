...
android_app_category
   How the app is categorized, should be one of "accessibility", "audio",
   "game", "image", "maps", "news", "productivity", "social", or "video".

android_debuggable
   A boolean indicating whether the app can be debugged. It is recommended to
   set this to True during testing.

android_min_sdk_version
   Overrides the lowest version of Android that the game will still operate on.
   The default is the minimum version of Android that Panda3D supports (21), it
   could make sense to increase this if your app uses additional APIs that are
...
android_manifest_file
   Path to an AndroidManifest.xml file that is included instead of the one
   generated by build_apps.  This can be used to further customize the app
   beyond what these settings permit. The settings in the file will override
   those specified in the build options. However, it is easy to get something
   wrong and break the app. Very little checking is performed to determine
   whether the provided file is consistent with the supplied build options and
   will result in a working build. It is recommended to first use build_apps to
   generate a working AndroidManifest.xml and then modify it further to suit
   your needs.
