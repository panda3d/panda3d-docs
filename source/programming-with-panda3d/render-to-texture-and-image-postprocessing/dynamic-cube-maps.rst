...
.. only:: python
    ...
    Panda3D makes this easy for you. To start rendering a dynamic cube map, simply
    call: 
    ...
    This will return
    an offscreen ``GraphicsBuffer`` that will
    ...
    Note that we passed a new
    ``NodePath``, called
    ``rig`` in the above example,
    to the ``makeCubeMap()`` call. This
    ``NodePath`` serves as the "camera
    rig"; the ``makeCubeMap()`` method will
    ...
    the shiny object and see the things that should be reflected in it.
    ...
    the ``MWorldCubeMap`` mode to generate
    texture coordinates for your geometry since the camera rig will have a
    :ref:`CompassEffect <compass-effects>` on it to keep it unrotated with respect
    to ``render``.
    ...
    your scene *six times* every frame in addition to the main frame render. If
    ...
    you can adjust the near and far plane of all of the cameras at once like this:
    ...
    the example performs very poorly on all but the highest-end hardware.)
    
    It's also usually a good idea to keep the cube map size (the
    ``size`` parameter to
    ``makeCubeMap()``) no larger than it
    absolutely has to be to get the look you want.
    
    You can also take advantage of the
    ``DrawMask`` to hide things from the
    ...
    ``buffer.setActive(0)``. Use
    ``buffer.setActive(1)`` to re-activate it.
